...
CTIP_CSV = DATA_DIR / "reference" / f"ctip_by_tract_{YEAR}.csv"
...
def test_programs_public_schema():
    """Programs file ships only public + syn-computed columns (no r1_noenroll)."""
    programs = pd.read_csv(PROGRAMS_CSV)
    assert "r1_noenroll" not in programs.columns
    expected = {
        "program_id",
        "school_id",
        "program_type",
    ...
    assert set(programs.columns) == expected
...
def test_geography_is_in_the_block_reference(students):
    ...
    assert "ctip_2013" not in blocks.columns
    ...
    ctip = pd.read_csv(CTIP_CSV)
    assert set(ctip.columns) == {"Tract", "ctip1"}
    assert set(ctip["ctip1"].unique()) <= {0, 1}
    tract_map = dict(
        zip(ctip["Tract"].astype("int64"), ctip["ctip1"].astype(float))
    )
    located = students.dropna(subset=["census_tract"])
    joined = located["census_tract"].astype("int64").map(tract_map)
    assert joined.notna().all()
    assert (located["ctip1"] == joined).all()


def test_priors_coarsened_scalars():
    """FRL edges must be snapped; unused district CTIP1 rate must not ship."""
    with open(PRIORS_JSON, encoding="utf-8") as handle:
        priors = json.load(handle)
    edges = priors["demographics"]["frl_bin_edges"]
    assert edges == [0.3, 0.6]
    assert "p_ctip1" not in priors
    assert "ctip_by_tract (binary CTIP1 per census tract)" in priors["meta"][
...
def test_regenerates_from_committed_priors(tmp_path):
    ...
    with open(PRIORS_JSON, encoding="utf-8") as handle:
        priors_meta = json.load(handle)["meta"]
    profile = priors_meta.get("profile", "joint_frl_eth")
    seed = module.PUBLIC_SEED
    ...
    shutil.copy(CTIP_CSV, work / "reference" / CTIP_CSV.name)
    shutil.copy(SCHOOLS_CSV, work / "Cleaned" / SCHOOLS_CSV.name)
    shutil.copy(PROGRAMS_CSV, work / PROGRAMS_CSV.name)

    module.generate(work, YEAR, seed=seed, profile=profile)
...
def test_cohort_size_and_nonempty_lists(students):
    """Public release targets the Table-1 / r1_filter universe."""
    assert len(students) == 3887
    lengths = students["r1_ranked_idschool"].apply(len)
