...
class PriorityGenerator:
    ...
    def _set_rounds_merged(self, rounds_merged: int) -> np.ndarray:
        ...
        return np.reshape(round_priorities, (self.market.n, 1)) * np.ones(
    ...
    def _set_policy_priorities(
        ...
        if self.market.config["grade"] == "06":
            return self._sixth_grade_priorities()
        if self.market.config["grade"] == "09":
            return self._ninth_grade_priorities()
    ...
    def _set_not_designation_priority(self, preferences):
        ...
        for i, row in enumerate(preferences):
            program_idxs = [
                int(x - 1)
                for x in row[: self.market.preference_generator.pref_length[i]]
            ]
            not_designation[i, program_idxs] = 1
