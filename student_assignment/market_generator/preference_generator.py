...
class PreferenceGenerator:
    ...
    def set_number_programs_ranked(self) -> np.ndarray:
        ...
        # YAML may parse bare numerics as int; list-length expressions are strings.
        ...
        # If option is a number, all students list option number of programs
            ...
            # High vs low FRL from freelunch_prob (0.5 cut).
            num_ranked_array = np.zeros(num_students)
            frl = student_data["freelunch_prob"].fillna(0)
            frl1 = student_data.loc[frl >= 0.5, "num_ranked"]
            frl1_length = frl1.sum() / frl1.count()

            frl0 = student_data.loc[frl < 0.5, "num_ranked"]
            frl0_length = frl0.sum() / frl0.count()

            for i in range(num_students):
                frl_score = frl.iloc[i]
                num_ranked_array[i] = (
