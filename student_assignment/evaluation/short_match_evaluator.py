...
class MatchEvaluator:
    ...
    def _entropy(self, ethnic_total_norm):
        """Entropy of a distribution (used by binary Theil H)."""
        props = ethnic_total_norm[ethnic_total_norm > 0]
        return float((props * np.log(1.0 / props)).sum())

    def _theil_from_matrix(self, ethnic_matrix, ethnic_total, ethnic_total_norm):
        """Theil H from a program/school-by-group count matrix."""
        district_entropy = self._entropy(ethnic_total_norm)
        total_students = ethnic_total.sum()
        if district_entropy == 0 or total_students == 0:
            return 0.0
        theil_sum = 0.0
        for _, row in ethnic_matrix.iterrows():
            school_total = row.sum()
            if school_total == 0:
                continue
            school_props = row / school_total
            school_entropy = self._entropy(school_props)
            theil_sum += school_total * (district_entropy - school_entropy)
        return float(theil_sum / (district_entropy * total_students))

    def _theil_group(self, group_students, all_assigned_students):
        """Theil H for one group vs the rest (binary), matching private Short_Match_Evaluator.

        Uses program-level (`assignment`) counts so High FRL Theil matches the paper row.
        """
        n_group = len(group_students)
        n_total = len(all_assigned_students)
        n_other = n_total - n_group
        if n_group == 0 or n_other == 0:
            return 0.0
        group_by_prog = group_students.groupby("assignment").size()
        total_by_prog = all_assigned_students.groupby("assignment").size()
        programs = total_by_prog.index
        in_group = group_by_prog.reindex(programs, fill_value=0).astype(int)
        other = (total_by_prog - in_group).reindex(programs, fill_value=0).astype(int)
        ethnic_matrix = pd.DataFrame(
            {"Group": in_group, "Other": other}, index=programs
        )
        ethnic_total = pd.Series({"Group": n_group, "Other": n_other})
        ethnic_total_norm = ethnic_total / ethnic_total.sum()
        return self._theil_from_matrix(
    ...
    def eval_assignment_paper_metrics(self):
        ...
        for ethnicity in ["Black", "Hispanic"]:
            ...
            # Paper Table 1: Black/White exposure to High FRL (vs assigned high-FRL peers)
            metrics[f"{ethnicity}/White exposure to high FRL"] = (
                self.metrics_segregation(
                    ethnic_students,
                    white_students,
                    high_frl_assigned,
        ...
        for group in groups + list(dico_income.keys()):
            ...
            metrics[f"Theil ({group})"] = self._theil_group(
                students, assigned_students
            )
            ...
            if group != "All Assigned":
                ...
                # All assigned (includes designated)
                metrics[f"Top 3 in-zone choice All Assigned ({group})"] = (
                    students["In-Zone Rank"] <= 3
                ).mean()
                # Convenience alias (not the Table 1 row — see PAPER_METRICS.md:
                # Rank Top 3 (non-CTIP) → Prop Top 3 choice (non-CTIP))
                if group == "non-CTIP":
                    metrics["Top 3 in-zone choice (non-CTIP)"] = metrics[
                        f"Top 3 in-zone choice All Assigned ({group})"
                    ]
