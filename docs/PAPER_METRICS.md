...
and written into the **Mean Values** sheet of `metrics_comparison.xlsx` by
`scripts/analysis/analyze_trends.py` (average over assignment iterations).
Pipeline context: [PIPELINE.md § Step 5](PIPELINE.md#step-5--aggregate-metrics).
...
| #Schools -15% High Income                                    | `#Schools with -15% High Income (95292)`       |
...
Many metrics also exist for subgroups (`CTIP`, ethnicity, FRL, …). Paper policy
tables typically use the **All Assigned** (or explicit non-CTIP) variant.
