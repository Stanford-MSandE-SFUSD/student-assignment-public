"""Generate a small synthetic dataset for pipeline smoke tests.

Produces year-consistent (2223) CSVs with the exact schemas the simulator
and evaluator expect:

  <out>/r1_filter_student_without_specialprogs_2223.csv
  <out>/programs_without_specialprogs_2223.csv
  <out>/Cleaned/schools_rehauled_2223.csv
  <out>/models/selectedsmall_2223_k1_prog_gesplit/estimates_2223.csv
...
tests/fixtures/small_2223/ and is exercised by tests/test_full_pipeline.py.

A larger realistic synthetic dataset may be added later; this fixture stays
intentionally tiny for CI.

Usage:
    python scripts/generators/generate_small_dataset.py \
        --out-dir tests/fixtures/small_2223 --num-students 200
"""
...
SMALL_YEAR_PREFIX = "2223"
SMALL_MODEL_NAME = "selectedsmall_2223_k1_prog_gesplit"
GRADE = "KG"
...
# Exact column order of the real r1_filter_student_without_specialprogs_*.csv files.
STUDENT_COLUMNS = [
    "studentno",
    "r1_ranked_idschool",
    "r1_listed_ranks",
...
def _make_students(
    ...
    """Build the r1_filter_student_without_specialprogs-style students table.
...
def generate_dataset(
    ...
    """Generate and save the small dataset.
    ...
    model_dir = out_dir / "models" / SMALL_MODEL_NAME
    zones_dir = out_dir / "zones"
    for directory in (out_dir, cleaned_dir, model_dir, zones_dir):
        directory.mkdir(parents=True, exist_ok=True)

    students_path = out_dir / f"r1_filter_student_without_specialprogs_{SMALL_YEAR_PREFIX}.csv"
    programs_path = (
        out_dir / f"programs_without_specialprogs_{SMALL_YEAR_PREFIX}.csv"
    )
    schools_path = cleaned_dir / f"schools_rehauled_{SMALL_YEAR_PREFIX}.csv"
    estimates_path = model_dir / f"estimates_{SMALL_YEAR_PREFIX}.csv"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Generate a small synthetic test dataset (year 2223)."
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("tests/fixtures/small_2223"),
        help="Output directory (default: tests/fixtures/small_2223)",
    )
    parser.add_argument(
        "--num-students", type=int, default=200, help="Number of synthetic students"
    )
    parser.add_argument(
